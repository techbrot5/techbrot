...
// Creates the Stripe Checkout Session AFTER acceptance evidence.
// All pre-payment evidence logic stays fully intact.
// ------------------------------------------------------------------
...
/* ------------------------------------------------------------------
 : MAIN — create session AFTER acceptance evidence saved
-------------------------------------------------------------------*/
export async function onRequestPost(context) {
  try {
    ...
    // ------------------------------------------------------------------
    // : 4. Convert acceptance-page lineItems
    // ------------------------------------------------------------------
    let items = [];
    ...
    const resolved = items.map(it => {
      ...
      return { ...it, billing, priceId, priceType, key: raw };
    });

    // ------------------------------------------------------------------
    // : 6. Query Stripe prices for metadata
    // ------------------------------------------------------------------
    const priceIdsToCheck = Array.from(
      new Set(
        resolved
          .map(r =>
    ...
    if (priceIdsToCheck.length) {
      ...
      await Promise.all(
        priceIdsToCheck.map(async pid => {
          try {
            priceInfo[pid] = await stripe.prices.retrieve(pid);
          } catch (e) {
            console.warn("Unable to retrieve Stripe price:", pid, e?.message);
          }
        })
      );
    ...
    const line_items = resolved.map(r => {
      ...
      if (!amount) {
        throw new Error(`Missing amount for: ${r.key}`);
      }
    ...
    const serverItemsMeta = JSON.stringify(
      resolved.map(r => ({
        ...
        billing: r.billing || r.interval || r.priceType
      }))
    ).slice(0, 480);
    ...
    const metadata = {
      ...
      plan_name: resolved
        ...
        .join(", ")
        .slice(0, 200),
      billing_intervals: resolved
        .map(r => r.billing || r.interval || r.priceType)
        .join(", ")
    };
