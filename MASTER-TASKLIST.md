...
- 🔵 **NEXT (autonomous, Wave D):** 3 core states WA·CO·MA (each: STATE-FACTS.md fact-checked vs that state's revenue dept FIRST → pillar + 5 essentials on the **PA/NJ/MD/VA/OH/MI/GA/NC/AZ/TN core-state exemplar**, route absent→global, Service+areaServed NO address, reviewedBy David on service children → battery on frozen copy → commit; checkpoint between states). Wave order: WA (starts Wave D) → CO → MA. Then national-hub capstone (drops the /find-an-accountant/ 301) → remaining T4 FAQ clusters → sitewide "verifiable on Intuit" honesty sweep (~50) → T6 flip gates (FULL battery, noindex-leak check, recrawl).
