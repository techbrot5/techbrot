...
- 🔵 **NEXT (autonomous, Wave D):** 2 core states CO·MA (each: STATE-FACTS.md fact-checked vs that state's revenue dept FIRST → pillar + 5 essentials on the **WA/TN/AZ core-state exemplar**, route absent→global, Service+areaServed NO address, reviewedBy David on service children → battery on frozen copy → commit; checkpoint between states). Wave order: CO → MA (final core). CO-distinct = flat 4.40% income tax + the home-rule city sales tax (~70 self-collecting cities, the standout hook) + low 2.9% state sales rate + retail delivery fee + PTE. MA-distinct = 5% flat income tax + the 4% millionaires surtax + the corporate excise (net-income + property/net-worth measure) + PFML + 6.25% sales tax. Then national-hub capstone (drops the /find-an-accountant/ 301) → remaining T4 FAQ clusters → sitewide "verifiable on Intuit" honesty sweep (~50) → T6 flip gates (FULL battery, noindex-leak check, recrawl).
