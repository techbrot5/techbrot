...
- 🔵 **NEXT (autonomous, Wave C cont'd):** 6 core states NC·AZ·TN·WA·CO·MA (each: STATE-FACTS.md fact-checked vs that state's revenue dept FIRST → pillar + 5 essentials on the **PA/NJ/MD/VA/OH/MI/GA core-state exemplar**, route absent→global, Service+areaServed NO address, reviewedBy David on service children → battery on frozen copy → commit; checkpoint between states). Wave order: NC·AZ·TN → WA·CO → MA. Then national-hub capstone (drops the /find-an-accountant/ 301) → remaining T4 FAQ clusters → T6 flip gates.
