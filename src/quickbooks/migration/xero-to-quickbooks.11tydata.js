...
module.exports = {
  ...
  faq: [
    ...
    { q: "How are bank transactions and reconciliation history handled?", a: "They&rsquo;re handled with opening balances, because Xero&rsquo;s reconciled bank transactions are tied to its own bank statement lines and don&rsquo;t translate cleanly into QuickBooks. We set a conversion date, pull Xero&rsquo;s trial balance as of that date, establish matching opening balances for every bank and credit-card account in QuickBooks, and reconcile the new file back to Xero before sign-off. Your first reconciliation in QuickBooks then works from a known-good starting balance. Prior-period bank detail stays in Xero as a read-only archive &mdash; there&rsquo;s rarely value in paying to re-key reconciliation history you can still look up." },
    ...
    { q: "Will my numbers be accurate after migrating from Xero?", a: "That&rsquo;s the entire point of doing it with a ProAdvisor. The migration includes verifying the opening balances against Xero&rsquo;s trial balance at the conversion date &mdash; balances, AR, AP, and account totals in QuickBooks are reconciled to the source before sign-off, so the new file ties back to the numbers you had in Xero. This source-tied verification is the discipline we call our Migration Integrity Protocol; it&rsquo;s the step a raw export skips, and the reason businesses that migrate carefully don&rsquo;t end up with balances they can&rsquo;t trust." },
  ],
  eleventyComputed: {
    pageGraph(data){
