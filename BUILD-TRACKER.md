...
**📐 BLUEPRINT SoT (2026-06-28): `techbrot-blueprint-v6.xlsx` (repo root) is the current source of truth; v5 retired to `_archive/`. It is built from `_build/reports/inventory.csv` (728 live pages, generated by `_build/scripts/inventory.py` from _site-prod). DRIFT GUARD: the battery's `inventory-drift` check (reads _site-prod) FAILS if any sitemap url is unbuilt, a NEW built-not-in-sitemap url appears outside `_build/reports/inventory-allowlist.txt`, or the prod page count != the committed inventory total. When pages are added/removed: `npm run inventory` → recommit inventory.csv → rebuild the blueprint in the strategy chat. Red-teamed (clean PASS / injected page FAIL). commit 8dcdabf. Memory: [[og-image-pipeline]] sibling tooling.**
