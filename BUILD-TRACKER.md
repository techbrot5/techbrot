...
**🔴 CUTOVER IN PROGRESS (2026-06-27) — preview-11ty IS the techbrot-project production branch; production deploy was failing, now serving 404. FIX #1 DONE: removed unused repo-root /functions (4 lead handlers; the Pages-Function publish step was failing every deploy; forms POST client-side to the external Apps Script /exec, NOT /functions — confirmed safe). commit d576937; that deploy then SUCCEEDED. FIX #2 PENDING (Cloudflare dashboard, FOUNDER-ONLY — not settable from repo): the successful deploy serves 404 because the project build config still targets old `main` ("Build command: None, output dir /"); preview-11ty is 11ty with NO root index.html — HTML is in _site-prod/ only after `npm run build:prod`. FOUNDER MUST SET in Cloudflare > Workers & Pages > techbrot > Settings > Builds & deployments: Build command=`npm run build:prod`, Build output directory=`_site-prod` (ENVIRONMENT=production already set), then Retry deployment. Added .nvmrc=20 (commit 2e1bd45). LOCAL PROOF: build:prod=734 files/727 sitemap/0 err, full battery 154 PASS, env gate both ways, forms render with Apps Script endpoint intact. AFTER dashboard fix + redeploy → verify techbrot.com=v2 (sitemap 727, no bootstrap, robots Allow, homepage index,follow, 4 redirects, forms submit). See SESSION-LOG 2026-06-27 cutover-fix turn.**
