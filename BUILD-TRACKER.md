...
**COUNT: total pages live = 471 · re-skinned (142) + growth = 329 · LAST COMMIT = round-77 QB-setup-payroll-pos
(6: setup/checklist, setup/new-business, setup/migration-from-spreadsheets, payroll/direct-deposit, payroll/tax-filing, migration/pos-migration). Working tree clean on `preview-11ty`.
GROUP 4 (task #19) ✅ COMPLETE — recon (6) + product (7) + setup (3) + payroll (2) + pos-migration (1) = 19 pages. NEXT: GROUP 5 = task #20 accounting service spokes (~12) → then task #21 tax-triage + speak-to-accountant + merges/301s (the last group). (payroll/tax-filing + direct-deposit honestly split Intuit-files/processes vs we-set-up-and-reconcile; pos-migration states QB Desktop POS discontinued.)
