...
**▶ NOW: founder review of NY/migration FAQ cuts → remaining T4 clusters (prose, pillars, location children, comparison, /faq/ hubs EXEMPT, t-hub ADDITIONS, silo links) → T5 LOCATIONS (LOCATION PLAN FIRST — HARD-STOP) → T6 flip gates (FULL battery run).** (standing rule in PRE-PUBLISH-AUDIT.md: title LEADS with keyword · brand `· TechBrot` only if ≤60 else DROP · unique · desc lead value+keyword ≤160 · phone on high-intent[done]). Scope: 263 titles >60 + 511 descs >160 (the 52 high-intent descs already tight from TASK 4). Per page-type, highest-value first, authored not bulk-stamped, battery+commit per wave. THEN:
