...
| **D4/D5/D6 (state children, ~90)** | t-location (v2) | **🔄 2/~90 (foundation)** | t-location v2 + 2 NY REPs TRUE-v2 (albany/nonprofit). .chips component. Fan-out = per-page body→v2 (sec.faqH + own schema anchors + rate-free viz), NY frozen, batch per state. ⚠ ~24 other t-location pages now transient chrome-only — recompose FIRST. Then bring ~64 t-bofu children onto v2. |
