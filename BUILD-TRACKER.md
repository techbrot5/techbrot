...
**▶ NOW: T4 FAQ (per FAQ-STRATEGY.md, money-first: answer-first rewrite + 1-2 call-intent FAQs w/ phone REPLACE-weakest-at-cap + t-hub additions + silo links) → T5 LOCATIONS (LOCATION PLAN FIRST — HARD-STOP for founder review) → T6 flip gates (incl one FULL battery run; --fast gates are provisional).** (standing rule in PRE-PUBLISH-AUDIT.md: title LEADS with keyword · brand `· TechBrot` only if ≤60 else DROP · unique · desc lead value+keyword ≤160 · phone on high-intent[done]). Scope: 263 titles >60 + 511 descs >160 (the 52 high-intent descs already tight from TASK 4). Per page-type, highest-value first, authored not bulk-stamped, battery+commit per wave. THEN:
