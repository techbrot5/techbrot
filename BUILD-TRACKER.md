...
## ⭐ V2 RECOMPOSE — CURRENT STATUS (updated 2026-06-26 · T5 WASHINGTON core state BUILT — Wave D started, 18/20 states · FULL battery green)
**🟢 T5 CHECKPOINT (2026-06-26) — WASHINGTON CORE STATE COMPLETE; WAVE D STARTED; FULL battery green (flip-gate mode), pushed. NEXT: Colorado → Massachusetts → national-hub capstone.**
**▶ NEXT-STATE GUARDRAIL: write `CO-FACTS.md` (vs Colorado Dept of Revenue) FIRST, then author 6 pages on the WA/TN exemplar.** CO-distinct = flat 4.40% income tax + the notoriously complex **home-rule city sales tax** (~70 self-collecting home-rule cities file separately from the state SUTS portal — the standout CO hook, analog to WA's city B&O) + state 2.9% sales tax (one of the lowest state rates, but combined is high after local) + retail delivery fee + SALT-parity PTE. Then MA (the 13th/final core state, pre-flip): 5% flat income tax + the 4% millionaires surtax (the MA-distinct hook) + the corporate excise (net-income + property/net-worth measure) + PFML + 6.25% sales tax. After MA: national-hub capstone (drops the /find-an-accountant/ 301) → T4 FAQ remainder → sitewide "verifiable on Intuit" honesty sweep (~50) → T6 flip gates.
...
**STATES: 18 built / 20 planned** (5 existing NY·CA·TX·FL·IL + DE + IN + PA + NJ + MD + VA + OH + MI + GA + NC + AZ + TN + **WA**; 2 core remain: CO·MA — ALL pre-flip). **NEXT: Colorado** (CO-FACTS.md vs Colorado Dept of Revenue FIRST → pillar + 5 essentials → battery → commit. CO-distinct = flat 4.40% income tax + the home-rule city sales tax [~70 self-collecting cities file separately, the standout hook] + low 2.9% state sales rate + retail delivery fee + PTE). Then MA (final core, pre-flip) → national-hub capstone → T4 FAQ remainder → honesty sweep → T6 flip.
