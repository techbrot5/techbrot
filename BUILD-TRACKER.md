...
**🟢 POST-CUTOVER PERF (2026-06-28, all live, battery 154 PASS) — sitewide template/build-level LCP fixes (NOT per-page). Baseline mobile 82 / LCP 4.2s. (1) SELF-HOST FONTS (commit 8f2b54e): dropped the Google Fonts CDN <link> from dc-base (Cloudflare was rewriting it to many static /cf-fonts/ subset files); now same-origin variable woff2 (Fraunces normal+italic, Geist, Spline Sans Mono, font-display:swap), only Fraunces+Geist preloaded (LCP H1 + body), italic+mono lazy. (2) ONE RENDER-BLOCKING CSS (commit 8130ee1): merged dc-system.css (was a 2nd render-blocking raw 57KB <link> on every page) INTO site.min.css as 99-dc-system.css (last layer, cascade preserved + now minified) → ONE file 107.4KB (total CSS dropped); gate raised 72→120KB (site.min.css.11ty.js + run_battery.py); css-drift exemption carried to 99-dc-system.css. (3) GTM OUT OF LCP PATH (8130ee1): GA4 now loads on first interaction OR ~2s after load (was eager async); keeps pageviews, removes ~66KB + 41ms reflow from render. dc-base + base both. AFTER PageSpeed pending founder re-run (pagespeed.web.dev mobile). Further lever if needed: inline critical CSS + async rest (not done). See SESSION-LOG 2026-06-28 perf turns.**
