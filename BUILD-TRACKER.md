...
## ⭐ V2 RECOMPOSE — CURRENT STATUS (updated 2026-06-26 · T5 TENNESSEE core state BUILT — Wave C (Sunbelt) COMPLETE, 17/20 states · FULL battery green)
**🟢 T5 CHECKPOINT (2026-06-26) — TENNESSEE CORE STATE COMPLETE; WAVE C (Sunbelt: GA·NC·AZ·TN) DONE; FULL battery green (flip-gate mode), pushed. NEXT: Washington (starts Wave D — WA·CO → MA → national-hub capstone).**
**▶ CONTEXT CHECKPOINT (2026-06-26, after TN): `WA-FACTS.md` ALREADY WRITTEN + VERIFIED + CITED** (vs WA Dept of Revenue `dor.wa.gov`, ESD `esd.wa.gov`, WA Cares `wacaresfund.wa.gov`) — the guardrail is DONE, so the next session authors WA's 6 pages straight from `_build/strategy/WA-FACTS.md` with NO re-verification needed. **WA-distinct = no income tax (no state withholding) BUT the B&O gross-receipts tax (by classification, NO cost deduction, + city B&O — the standout hook, like AZ's TPT) + high combined sales/use tax (6.5% state + local) + the 7% capital-gains tax on owners (advisory; $278k 2025 deduction, +2.9% over $1M; real estate/retirement/business-use exempt) + the WA payroll premiums (PFML 1.13% for 2026, employer 28.57%/employee 71.43%, <50 employees exempt from employer share; WA Cares 0.58% employee-paid).** Build per the TN/AZ exemplar (pillar `washington.*` slug `wa` intent `washington`, sameAs `Washington_(state)`; 5 children wa-svc-*; areaServed-only NO address; related = California·Texas·Tennessee EXIST + Oregon/Idaho/Nevada coming-soon, weave the OR-no-sales-tax border nuance). Then register_state.py washington → build → battery-frozen → commit. **Stopped here to honor "never start a state that would exhaust context mid-build" — TN was a full 12-file build this turn.**
...
**STATES: 17 built / 20 planned** (5 existing NY·CA·TX·FL·IL + DE + IN + PA + NJ + MD + VA + OH + MI + GA + NC + AZ + **TN**; 3 core remain: WA·CO·MA — ALL pre-flip). **NEXT: Washington** (WA-FACTS.md vs WA Dept of Revenue FIRST → pillar + 5 essentials → battery → commit; STARTS Wave D. WA-distinct = NO personal income tax but the **B&O gross-receipts tax** [by classification, no deduction for costs] + high combined **sales/use tax** + the **2022 capital-gains tax** on individuals + Paid Family/Medical Leave + WA Cares payroll).
