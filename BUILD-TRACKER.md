...
## ⭐ V2 RECOMPOSE — CURRENT STATUS (updated 2026-06-26 · 🎉 ALL 20 STATES BUILT — MASSACHUSETTS (final core) DONE, Wave D COMPLETE, 20/20 · FULL battery green)
**🟢 OG STRUCTURED-DATA FIX (2026-06-28, live, battery 154 PASS) — founder caught /assets/img/og/techbrot-og.png (404, never built). It was NOT an og:image fallback (meta already uses the new cards) but a leftover referenced ONLY in JSON-LD: schemaGlobal "image" + 226 per-page primaryImageOfPage, all -> 404. fix_og_schema.py repointed all primaryImageOfPage -> the page's real /assets/og/cards/<slug>.png (== og:image; built-site broken->card map) + contact.11tydata.js template-literal fix + schemaGlobal image -> /assets/og/default.png. 0 /assets/img/og refs left anywhere; LIVE-verified primaryImageOfPage resolves 200, schemaGlobal image=default.png. Every page now has its correct OG image in BOTH meta and structured data. commit d2e1ab0.**
**🟢 OG IMAGES DONE (2026-06-28, all live, battery 154 PASS) — templated per-page social cards for all 728 pages. Founder-approved template = Direction B "Split Ledger" (ink text panel + petrol monogram panel, amber divider/ledger motif). Pipeline (build-time, not run by Cloudflare): og_fonts.py instances the variable brand woff2 -> static TTF; og-gen.mjs (satori -> sharp) reads each built og:title + derives category from the URL silo, renders 1200x630 PNG (text vectorized to paths). `ogCard` eleventy filter + per-page og:image/twitter:image in dc-base+base. 728 cards in src/assets/og/cards (avg 12.9KB). VERIFIED 728/728 wired to correct existing card, twitter==og, 1200x630; LIVE og:image=/assets/og/cards/home.png, cards 200 image/png 1200x630. Re-run og-gen.mjs for new pages. commit 060e711. Memory: [[og-image-pipeline]]. TODO 9 (OG batch go-remote gate item) CLOSED.**
**🟢 POST-CUTOVER PERF (2026-06-28, all live, battery 154 PASS) — sitewide template/build-level LCP fixes (NOT per-page). Baseline mobile 82 / LCP 4.2s. (1) SELF-HOST FONTS (commit 8f2b54e): dropped the Google Fonts CDN <link> from dc-base (Cloudflare was rewriting it to many static /cf-fonts/ subset files); now same-origin variable woff2 (Fraunces normal+italic, Geist, Spline Sans Mono, font-display:swap), only Fraunces+Geist preloaded (LCP H1 + body), italic+mono lazy. (2) ONE RENDER-BLOCKING CSS (commit 8130ee1): merged dc-system.css (was a 2nd render-blocking raw 57KB <link> on every page) INTO site.min.css as 99-dc-system.css (last layer, cascade preserved + now minified) → ONE file 107.4KB (total CSS dropped); gate raised 72→120KB (site.min.css.11ty.js + run_battery.py); css-drift exemption carried to 99-dc-system.css. (3) GTM OUT OF LCP PATH (8130ee1): GA4 now loads on first interaction OR ~2s after load (was eager async); keeps pageviews, removes ~66KB + 41ms reflow from render. dc-base + base both. RESULT (founder pagespeed.web.dev mobile): 82/LCP 4.2s -> 94/LCP 2.1s (+12 perf, LCP -50%, now under the 2.5s Good threshold). Further lever if ever needed: inline critical CSS + async rest (not done -- not needed at 94). See SESSION-LOG 2026-06-28 perf turns.**
**🎉 CUTOVER COMPLETE (2026-06-28) — techbrot.com IS LIVE on the v2 11ty site, indexable. Founder set dashboard Build command=`npm run build:prod`; with preview-11ty's wrangler.toml (pages_build_output_dir=_site-prod) the deploy built + published _site-prod. LIVE VERIFICATION 7/7 PASS: (1) homepage v2 + index,follow, 0 bootstrap; (2) robots.txt Allow:/ + sitemap ref; (3) sitemap 727 (was 112); (4) NY pillar + money + /contact/ + /quickbooks/file-review/ all 200 + index,follow; (5) 4 redirects all 301 to correct targets (/about/team/→/about/, 3 unbuilt states→hub); (6) forms LIVE — TB_ENV=production + Apps Script /exec on contact + file-review; (7) no Bootstrap bleed-through (old spam slugs 404, /blog/ is legit v2, deep pages dc-system only). DEPLOY PATH LOCKED: push to preview-11ty → Cloudflare runs npm run build:prod → publishes _site-prod. EVERY PUSH IS NOW PRODUCTION. ✅ MAIN CLEANED UP (2026-06-28): old v1 main (Bootstrap + Stripe funcs + "." wrangler.toml + wrangler cache) archived to git tag `v1-bootstrap-archive` (commit bbae9ce, history preserved), then main force-overwritten to mirror preview-11ty's clean v2 tree (now 0 functions/, 0 Stripe, 0 wrangler cache, wrangler.toml=_site-prod). preview-11ty = SOLE production source; main = clean fallback snapshot, no cruft to redeploy. Post-cutover optional remaining: self-host fonts; live PageSpeed/axe; (founder's call) set GitHub default branch to preview-11ty + delete main. See SESSION-LOG 2026-06-28 cutover-complete + main-cleanup turns.**
<!-- Historical (the fix path):
**🔴 CUTOVER IN PROGRESS (2026-06-28) — preview-11ty IS the techbrot-project production branch. ROOT CAUSE of the 404 FOUND + FIXED in repo: the project was wrangler-set-up; `main`'s wrangler.toml has `pages_build_output_dir = "."` (serve repo root, no build) — correct for v1 Bootstrap (HTML at root), FATAL for 11ty (preview-11ty has NO root index.html; output is _site-prod/ after build) → Cloudflare published the bare root → 404 (and the dashboard build-output field read blank/uneditable because wrangler.toml manages it). FIXES DONE: (1) removed unused /functions (commit d576937); (2) .nvmrc=20 (2e1bd45); (3) Path A — added a CLEAN preview-11ty wrangler.toml with `pages_build_output_dir = "_site-prod"` overriding the broken "." (commit a72e85f). preview-11ty is clean — NO v1 Stripe/Postmark/checkout functions, NO old wrangler cache, only the minimal config + the 11ty site. REMAINING (founder, dashboard, ONE field): set Build command = `npm run build:prod` (wrangler.toml can't carry the build command for Pages Git builds; output dir now managed by wrangler.toml=_site-prod), Save → Retry deployment on a72e85f. LOCAL PROOF: build:prod=734 files/727 sitemap/0 err, full battery 154 PASS, env gate both ways, forms render with Apps Script endpoint intact. THEN verify techbrot.com=v2 (sitemap 727, no bootstrap, robots Allow, index,follow, redirects, forms). See SESSION-LOG 2026-06-28 turn.**
-->
**🟢 T5 MILESTONE (2026-06-26) — 🎉 ALL 20 STATES BUILT + NATIONAL-HUB CAPSTONE DONE; FULL battery green (flip-gate mode), pushed. The location silo + its hub are COMPLETE.**
**✅ NATIONAL-HUB CAPSTONE DONE (2026-06-26, commit 6e14137, battery green):** (1) `/find-an-accountant/` hub (`index.njk` + `index.11tydata.js`) now lists **all 20 built states** — the `builtOut` selector map (drives the 50-state dropdown → 20 marked "dedicated page"), the routing-card grid (20 cards, each with its REAL verified local-tax hook), the "20 states built out" note, the network card, both FAQ answers, the meta description, and the CollectionPage schema description all updated from the stale 5-state (NY/CA/TX/FL/IL) framing. (2) **DROPPED 13 stale `/find-an-accountant/<state>/` 301s** in `src/redirects.njk` (arizona, colorado, georgia, indiana, maryland, massachusetts, michigan, new-jersey, north-carolina, ohio, pennsylvania, virginia, washington) — these were 2026-06-21 flip-gate placeholders that would now SHADOW the real built pages; only the 3 states still without a page remain 301'd to the hub (alabama, minnesota, nevada). URL set UNCHANGED (728/728 non-dev, identical to the MA commit — no pages added/removed; only hub content + redirect rules changed). Battery FULL flip-gate green; only 3 files touched.
**✅ "VERIFIABLE ON INTUIT" HONESTY SWEEP DONE (2026-06-26, commit 00944fb, battery green):** softened ~90 "verifiable on Intuit('s directory)" overclaims sitewide OUTSIDE the location silo (the silo was already done) to the approved general form — "Intuit&rsquo;s public ProAdvisor directory lists active ProAdvisors for verification" — removing the implication that TechBrot is INDIVIDUALLY findable/searchable there. Covered: ~24 error-code data files (cert line + the annual re-cert stat label → "Annual Intuit re-certification"), accounting/pricing/industries trust-row labels + delivery cards, quickbooks/help, proadvisor-near-me, proadvisor-team (light touch — general credential explanations KEPT), platforms/quickbooks (light touch), quickbooks/support/*, speak-to-a-quickbooks-expert, _data/glossary.js, and the **trust page** (incl 2 baseline FAQ answers — questions kept byte-identical, only the "searchable by name/firm" overclaim in the answers corrected + the real "we&rsquo;ll walk you through it on a call" path added; /trust/ content-equity stayed GREEN). Real cert facts (QBO L2/Desktop/Enterprise/Payroll, annual renewal) and all "not affiliated with Intuit" lines untouched. **Final grep: 0 overclaim in production src** (excl `/dev/` kitchen-sink fixtures, which are noindex/non-production and intentionally left). 3 helper scripts in `_build/scripts/` (honesty-sweep-errorcodes/validate, soften-recert-label). Done via 2 subagents + my hand-fix of the 2 trust baseline FAQs.
**✅ PILLAR HERO ABOVE-FOLD SHORTENED (2026-06-26, commit bab40ca, battery green):** `t-state-pillar.njk` layout now renders a SHORT conversion-first hero intro (generic line + state name from the breadcrumb tail → all 20 inherit) instead of the long `hero.subheading`; the dense tax depth stays in `summarized` + stats + glossary (no dup, no loss). NY content-equity green.
**▶ CHILD HERO finding (founder asked to check D4/D5/D6):** t-money STATE SERVICE CHILDREN have long above-fold intros (806–957 chars) vs non-state money pages already fine (~210–246). t-money is SHARED → per-page pass on the state children only (75 core-state children + DE/IN/NY/landing children to assess), equity- & depth-safe (depth already in §01 in-brief). Subagent pass next; see MASTER-TASKLIST.
**✅ T4 FAQ — HUB ADDITIONS + COMPARISON DONE (2026-06-26, commits f0ecaf5 + aa8ca90, battery green):** hub additions = purely-additive Option-C Qs on 8 under-cap hubs (accounting/quickbooks→10, advisory/industries/compare/find-an-accountant/switch→9, platforms→8; /vs/ skipped — no FAQ render slot). comparison = 21 pages (7 qb/compare + 14 /vs/, at cap) answer-only: phone into a decision-answer + silo links, 0 questions touched, 58,051 hrefs resolve. content-equity all PASS both.
**✅ T4 FAQ PROSE WAVE 1 (service prose ~55 pages, commit 4371595) + SECTION-EYEBROW FIX (§NN dropped sitewide → bare §, 94 files, commit 980660e) DONE (2026-06-27, batteries green).** Prose wave 1 = silo interlinks + selective phone on accounting service / advisory / qb cleanup+payroll+setup+service prose; answer-only, 0 questions touched, 58,179 hrefs resolve. Eyebrow fix = `_build/scripts/drop_eyebrow_numbers.py`, equity-safe, variety-12a green.
**✅ T4 FAQ PROSE WAVE 2+ DONE (2026-06-27) — 🟢 CONTENT WORK COMPLETE, FLIP-READY (pending founder go on T6).** 3 commits (Wave A f5f4b45 · Wave B 5acd32b · final sweep), each FULL battery green. ~176 prose pages enriched: Option-C answer-only silo interlinks sitewide + SELECTIVE phone on genuine call-intent only (qb/help 8 urgent, error-codes 6 data-loss, pricing+switch 14, industries 26 [one each — FLAGGED to founder], app-stack/which-software 2). qb/online + resources + tools + enterprise/desktop = silo-links-only (no phone — informational). Zero FAQ questions touched; 58583 hrefs resolve; content-equity all PASS; meta-unique 728/728. **Plus: HUB-HERO SHORTENING DONE (commit 2153e98) — 23 over-long t-hub heroes → tight ~210-270 char leads per the founder hero-length rule (hubs SHORT, all else FULL, pillars as-done). CHILD-HERO pass CANCELLED (founder 2026-06-27). TRUST-STRIPS = proposed+gated (awaiting founder ruling).**
**✅ POST-WAVE-2 BLOCK DONE (2026-06-27): industries phone REVERTED (342f0d4) · FAQ flip-gate audit A–F + 4 schema-match fixes → verbatim 668/668 (59ad54f) · trust strips CLEAR-CUT applied (ccfaf72: comparison both, service hubs badge, pricing both, pillars unchanged, informational none). ✅ SERVICE-PROSE trust-strip BOTH group DONE (be97e83, founder-confirmed 50; borderline 10 = none; industries-hub phone stripped). 🎉 ALL CONTENT + TRUST-STRIP WORK COMPLETE. T6 flip UNTOUCHED.**
**✅ CONVERSION ENDPOINTS DONE (2026-06-27, commit 9036e71, battery green): minimal nav/footer chrome on /contact/ + /quickbooks/file-review/ (minimalChrome flag in dc-base — drops topbar/mega-nav/hamburger/breadcrumb/5-col-footer → brand+phone+CTA + minimal footer; form/funnel verified intact). Contact ?intent= personalization AUDITED (intent_audit.py) = comprehensively wired, 157 defined / 93 used / 0 undefined-fallback, all decision page-types covered — NO gaps. Part-2 (footer-per-family, contact redesign) = design, deferred.**
**✅ CONTACT PAGE REORDER DONE (2026-06-27, commit 2ce2f03, battery green): form moved BELOW the reassurance sections (hero → how-to-reach → why-owners → form → other-ways; trust before the ask). whyBullets render audit = NO bug in code (5 items server-rendered, intent-independent, intent JS never touches the checklist, CSS correct, no mobile/reveal hide) — items render on all variants; flagged for a specific repro if still seen. Form/funnel intact.**
**✅ CONTACT PAGE TRIMS DONE (2026-06-27, commit ddad46b, battery green): whyBullets 5→3 (kept Certified×4 / fixed-fee-written-first / named-operator) + contact FAQ 7→5 (kept discovery-cost / sales-pitch / not-right-fit / pricing-after-call / urgent-skip-form). FAQ is ADDITIVE (baseline 0 FAQ) so not equity-gated; schema maps data.faq so visible+JSON-LD trimmed together → POST-TRIM verbatim 668/668 MATCH, /contact/ visible==schema exactly, content-equity PASS.**
**✅ ENDPOINT CTA CLEANUP DONE (2026-06-27, commit ca00319, battery green): /contact/ + /quickbooks/file-review/ are now single-minded — minimal header = PHONE ONLY (dropped Speak-to-an-advisor head-cta; phone made always-visible by removing demo-collapse so mobile keeps it), hero down to ONE primary CTA each (removed the Speak-to-a-ProAdvisor btn--ghost), ZERO dead links (anchor targets + all hrefs resolve). Form + intent personalization verified intact.**
**✅ INTERNAL-LINKING AUDIT + FIX DONE (2026-06-27, commits cb2aa6c + 64e00d3, battery green): `_build/scripts/link_audit.py` (sitewide inbound-graph). Batch A routing: new `_data/builtStates.json` + t-state-pillar resolves `related` neighbors to REAL pillars (fixed 19 stale state fallbacks across 8 pillars — the screenshot bug); homepage state grid routes via builtStates (fixed 3 unbuilt-state 301-hops). Batch B orphans: 28 genuine orphans linked into their silos (new All-QB-help-topics directory, error-code hub completion, cleanup scenarios, money-page hub links, manhattan, reviews, frameworks, advisory/partners FAQ). RE-AUDIT CLEAN: 0 orphans, 0 stale fallbacks, 0 real broken links. Pillar→children was already healthy.**
**✅ HONESTY: DE PHYSICAL-OFFICE / LOCALBUSINESS / MAP-PACK OVERCLAIM REMOVED (2026-06-27, commit a68debd, battery green): founder ruled TechBrot has NO physical office (Middletown = DE C-corp legal address only). Deleted the DE pillar [AccountingService,LocalBusiness] node + hasMap/geo/openingHours → DE now Service+areaServed like the other 19; reframed every visible office/HQ claim to remote-service framing. Global #organization de-physicalized (dropped ProfessionalService + geo + openingHours sitewide; corporate PostalAddress kept as legal/contact). Sitewide sweep: 0 hasMap/GeoCoordinates/LocalBusiness on any production page. Battery GLOBAL_SCHEMA_REMOVALS registers the 3 authorized global removals as JUSTIFIED.**
**▶ NEXT: T6 FLIP GATES (FOUNDER-GATED — do NOT flip autonomously; founder authorizes flip prep next). All pre-flip functional/conversion + SEO/linking + honesty items complete. Superseded next-line (historical): T4 FAQ PROSE WAVE 2+ (industries 26 · qb/online 40 · qb/help 52 · resources 19 · tools 9 · enterprise/recon/desktop · find-an-accountant prose 7) → child-hero (CANCELLED) → T6.** PROSE SCOPE FINDING: 245/256 t-prose already have FAQs; the real work is silo interlinks + SELECTIVE phone on call-intent prose only (do NOT mass-weave phone into 233 info FAQs = forbidden stuffing); triage by value, subagents, content-equity to completion+READ per commit. See MASTER-TASKLIST for the full prose/child-hero plan. T6 FLIP remains (FULL battery, noindex-leak check, baseline-vs-prod recrawl, prod-env proof, robots/sitemap flip). ⚠ THE T6 FLIP IS THE PRODUCTION GO-LIVE — outward-facing + hard to reverse → REQUIRES explicit founder confirmation before flipping robots/sitemap/ENVIRONMENT=production; do NOT flip autonomously.** Deferred cosmetic: /dev/ kitchen-sink fixtures still carry the old verify phrasing (noindex, non-production). State-build exemplars: built states under `src/find-an-accountant/<state>/` + `_build/strategy/<XX>-FACTS.md`.
**▶ AUTONOMOUS RUN AUTHORIZED (founder GO 2026-06-26): build all 13 core states → capstone → T4 FAQ remainder → T6 flip.**
**MA RULED IN (founder 2026-06-26):** MA = 13th core state, **PRE-FLIP build** (supersedes earlier "MA deferred"). **20 states total at flip.** LOCATION-SEO-STRATEGY.md #1 + build order updated. Wave order: PA·NJ·MD·VA → OH·MI → GA·NC·AZ·TN → WA·CO → MA.
**STANDING RULES CONFIRMED RECORDED (founder 2026-06-26):** per-state FACTS.md verified+cited BEFORE authoring (the one real risk = invented tax facts) · content-equity to completion + READ before every commit · auto-checkpoint at ~75-80% context (never start a state that would exhaust context mid-build; half-built state = STOP) · honest schema only (no fake addresses/reviews/credentials) · data-files-only on existing v2 layouts · battery + commit per state · update MASTER-TASKLIST+SESSION-LOG+BUILD-TRACKER+strategy docs every turn · hard-stop on honesty/depth-loss/missing-component/context-full. **ASSIGNED:** honesty sweep of the ~50 sitewide "verifiable on Intuit" occurrences OUTSIDE the silo (founder keeps DE-GBP himself).
**🟢 PA CORE STATE COMPLETE (2026-06-26) — FULL battery green, pushed.** Pennsylvania = **6 pages** (pillar + 5 essentials: quickbooks-accountant · small-business-accountant · quickbooks-cleanup · quickbooks-setup · bookkeeping-services). **PA is the FIRST core state — it sets the CORE-STATE PATTERN** (exemplar for the remaining 12): t-state-pillar sections are `{% if %}`-guarded, so a core pillar OMITS `ecosystem`, routes `industries` cards → GLOBAL `/accounting/industries/*`, lists `cities` with NO href (metros named for areaServed, no city pages), and pricing rows → 5 children + global. Built from `_build/strategy/PA-FACTS.md` (fact-checked vs PA DOR + PA DCED + City of Philadelphia: flat **3.07%** PIT · **Act 32 local EIT** by **PSD code** under the "higher-of" rule [rates QUALITATIVE] · **LST** ≤$52/yr · **CNIT 7.49%** 2026→4.99% 2031 · sales tax **6%/7% Allegheny/8% Philly** · **Philadelphia** Wage 3.74%/3.43% + NPT + BIRT 5.71%+1.410mills, $100k excl gone TY2025+). areaServed-only, NO street address. The PA-distinct service hook = Act 32 EIT/PSD + Philadelphia city taxes + split sales tax (analog of IN's county LIT). 5 children fanned out via subagents, data-files-only. **Battery GREEN** (frozen copy via new `_build/scripts/battery-frozen.ps1`): links 51363/688 resolve, meta-unique 656, content-equity all PASS (additive). Bug caught+fixed: an escaped `\"` href in cleanup.njk BODY prose (correct in JSON/JS, wrong in template HTML) → fixed. register_state.py pennsylvania ran.
**🟢 NJ CORE STATE COMPLETE (2026-06-26) — FULL battery green, pushed.** New Jersey = **6 pages** (pillar + 5 essentials) built on the PA core-state exemplar (subagents used the same-named PA children). From `_build/strategy/NJ-FACTS.md` (fact-checked vs NJ Division of Taxation + NJ DOL): **graduated GIT 1.4%→10.75%** over $1M (vs flat PA/IN) · **PA–NJ reciprocity** (Form NJ-165 — ties to the PA anchor) · **the NJ payroll stack** UI/TDI/FLI/WF, employer+employee, **two 2026 wage bases $44,800/$171,100** (employee TDI 0.19%/FLI 0.23%) — the NJ-distinct hook · CBT 9% + **Corporate Transit Fee 2.5%** (>$10M, 2024–2028) · sales tax **6.625%** + **UEZ/Salem 3.3125%** half-rate · **BAIT** (advisory) · **Newark/Jersey City employer payroll taxes**. areaServed-only, NO address. Battery GREEN (frozen copy): links 51906/694 resolve, meta-unique 662, content-equity all PASS. 0 escaped-quote bugs (subagents warned off the PA issue), 0 dead links. register_state.py new-jersey ran.
**🟢 MD CORE STATE COMPLETE (2026-06-26) — FULL battery green, pushed.** Maryland = **6 pages** (pillar + 5 essentials) on the PA/NJ exemplar (subagents used the same-named NJ children). From `_build/strategy/MD-FACTS.md` (fact-checked vs Comptroller of Maryland + BRFA 2025): graduated state tax **2%→6.50%** (new 2025 brackets) + **2% cap-gains surtax** (>$300k AGI) · **county "piggyback" Local Income Tax** (23 counties + Baltimore City, by MW507 county of residence — the MD-distinct hook) · **DC/PA/VA/WV reciprocity** (MW507M — ties to PA + upcoming VA) · **NEW 3% sales tax on data/IT/SaaS-B2B** (eff July 1 2025, vs 6% standard, MPU) · corporate 8.25% · **PTE election** (advisory). areaServed-only, NO address. Battery GREEN (frozen): links 52446/700 resolve, meta-unique 668, content-equity all PASS. 0 escaped-quote bugs, 0 dead links. register_state.py maryland ran. **Halfway: 10/20 states.**
**🟢 VA CORE STATE COMPLETE (2026-06-26) — FULL battery green, pushed. WAVE A (Mid-Atlantic PA·NJ·MD·VA) DONE.** Virginia = **6 pages** (pillar + 5 essentials) on the MD/NJ exemplar. From `_build/strategy/VA-FACTS.md` (fact-checked vs Virginia Dept of Taxation): state income tax 2%–5.75% + **NO local income tax** (the key contrast) · corporate 6% · **BPOL local gross-receipts business tax** (tracked by locality — the VA-distinct hook, NOT a payroll local tax) + BTPP/M&T · **variable regional sales tax** 5.3/6.0/7.0% · **MD/PA/WV/KY/DC reciprocity** (VA-4 — ties to MD+PA) · PTET (confirm 2026) · Data Center Alley exemption. areaServed-only, NO address. Subagents deliberately differentiated (no county-piggyback import). Battery GREEN (frozen): links 52983/706 resolve, meta-unique 674, content-equity all PASS. 0 escaped-quote bugs, 0 dead links. register_state.py virginia ran.
**🟢 OH CORE STATE COMPLETE (2026-06-26) — FULL battery green, pushed. WAVE B (Midwest) STARTED.** Ohio = **6 pages** (pillar + 5 essentials) on the MD/PA exemplar. From `_build/strategy/OH-FACTS.md` (fact-checked vs Ohio Dept of Taxation + ORC + HB96): **the municipal income tax** (649 cities + 199 school districts, work-location withholding, 20-day rule, RITA/CCA — the OH-distinct hook, the most complex local payroll tax in the US) · **NO corporate income tax** (the **CAT** gross-receipts tax replaced it; $6M exclusion 2026, 0.26%) · state income tax low/$26,050 exempt/**zero by 2030** (HB96, qualitative) · **IN/PA/MI/KY/WV reciprocity** (IT 4NR — state only, municipal still applies; ties to IN+PA) · county-variable sales tax (6.5–8.0%) · PTET (IT 4738). areaServed-only, NO address. Battery GREEN (frozen): links 53525/712 resolve, meta-unique 680, content-equity all PASS. 0 escaped-quote bugs, 0 dead links. register_state.py ohio ran.
**🟢 MI CORE STATE COMPLETE (2026-06-26) — FULL battery green, pushed. WAVE B (Midwest OH·MI) DONE.** Michigan = **6 pages** (pillar + 5 essentials) on the OH/MD core-state exemplar. From `_build/strategy/MI-FACTS.md` (fact-checked vs Michigan Dept of Treasury): **FLAT 4.25% individual income tax** (confirmed for 2026 — the clean contrast to OH's phase-out + the graduated NJ/MD/VA ladders) · **24-city CITA city income tax** withheld by work location — **Detroit 2.4%/1.2%** (VERIFIED), Grand Rapids/Saginaw 1.5%/0.75%, Highland Park 2%/1%, rest 1%/0.5%; Detroit admin'd by Treasury, Flint joins Treasury admin 2027; NO school-district layer, NO 20-day rule (the MI-distinct hook — analog to OH's municipal tax but far simpler) · a **REAL 6% Corporate Income Tax** on C-corps (vs OH's no-corp-tax/CAT) · **elective FTE tax** at 4.25% (Form 5772, SALT-cap workaround) · **flat 6% sales/use tax, NO local add-on anywhere** (vs OH's county-variable) · **IL/IN/KY/MN/OH/WI reciprocity** (Form MI-W4; STATE wages only — city tax still applies; ties to the OH+IN+IL anchors). areaServed-only, NO address. 5 children fanned out via 5 parallel subagents, data-files-only. **Battery GREEN** (frozen copy): meta-unique 686, content-equity all PASS (additive), layout-v2 686, design-fidelity 680, no mojibake/entities, 0 escaped-quote bugs, 0 dead links. register_state.py michigan ran (6 URLs queued; michigan + michigan-advisory intents). **13/20 states.**
**🟢 GA CORE STATE COMPLETE (2026-06-26) — FULL battery green, pushed. WAVE C (Sunbelt) STARTED.** Georgia = **6 pages** (pillar + 5 essentials) on the MI/OH core-state exemplar. From `_build/strategy/GA-FACTS.md` (fact-checked vs Georgia Dept of Revenue + Office of the Governor): **FLAT 4.99% income tax for 2026** (HB 463 lowered it from 5.19%; HB 1437 made GA flat; path to further reductions — the clean contrast to the graduated ladders and OH's phase-out) · **FLAT 4.99% corporate income tax** PLUS the **corporate NET WORTH TAX** (franchise-style: $0 if net worth ≤$100k but still files; graduated to a **$5,000 max over $22M**; Form 600 — the GA-distinct business tax) · **4% state sales tax + county LOCAL-OPTION add-ons** (LOST/SPLOST/ELOST/TSPLOST, Atlanta MOST; combined **~7–8%, jurisdiction-variable**, quarterly — the GA sales-tax hook) · **NO local/city income tax BUT NO reciprocity** (state-only withholding on **all** GA-source wages — the distinct contrast to OH municipal + MI city tax, and to the IL/IN/OH reciprocity states) · **PTE election** (HB 149) at 4.99% (advisory) · Atlanta logistics/fintech/film + Port of Savannah economy. areaServed-only, NO address. 5 children fanned out via 5 parallel subagents (MI children as exemplars), data-files-only. **Battery GREEN** (frozen copy). register_state.py georgia ran (6 URLs queued; georgia + georgia-advisory intents). **14/20 states.**
**🟢 NC CORE STATE COMPLETE (2026-06-26) — FULL battery green, pushed. Wave C (Sunbelt) continuing.** North Carolina = **6 pages** (pillar + 5 essentials) on the GA/MI core-state exemplar. From `_build/strategy/NC-FACTS.md` (fact-checked vs NC Dept of Revenue): **FLAT 3.99% income tax for 2026** (down from 4.25%; still phasing down via triggers — the lowest flat rate in the Sunbelt set) · **corporate income tax being PHASED OUT TO ZERO** (the NC-distinct headline — NC is the only state eliminating it; **2.00% for 2026 → zero by 2030**) · **the FRANCHISE TAX survives it** ($1.50/$1,000 of tax base, $200 min, $500 cap on first $1M, holding cos capped $150,000; CD-405/CD-401S — the NC business-tax hook, analog to GA's net worth tax) · **4.75% state sales tax + county (2.00/2.25%) + transit (0.50%, 4 counties)** = combined 6.75–7.50%, **Mecklenburg/Charlotte → 8.25% July 1 2026** (county-variable) · **NO local income tax BUT NO reciprocity** (state-only withholding on all NC-source wages; like GA) · **Taxed PTE election** (G.S. 105-154.1; advisory) · Charlotte banking/fintech + Research Triangle tech/biotech economy. areaServed-only, NO address. 5 children fanned out via 5 parallel subagents (GA children as exemplars), data-files-only. **Battery GREEN** (frozen copy). register_state.py north-carolina ran (6 URLs queued; north-carolina + north-carolina-advisory intents). **15/20 states.**
**🟢 AZ CORE STATE COMPLETE (2026-06-26) — FULL battery green, pushed. Wave C (Sunbelt) continuing.** Arizona = **6 pages** (pillar + 5 essentials) on the GA/NC core-state exemplar. From `_build/strategy/AZ-FACTS.md` (fact-checked vs Arizona Dept of Revenue): **FLAT 2.5% income tax** (the LOWEST flat income-tax rate of any US state, since 2023) · **4.9% corporate income tax** + elective **PTE at 2.5%** · **the Transaction Privilege Tax (TPT) — the AZ-distinct hook, NOT an ordinary sales tax**: a tax on the **seller's privilege of doing business**, owed by the vendor, levied **by business classification** (state 5.6% + county + city via the centralized ADOR/AZTaxes.gov portal) · **PRIME CONTRACTING** — construction taxed on **65% of gross receipts** (the prime contractor owes it, not subs; MRRA taxed differently — the famous AZ construction quirk) · **NO local/city income tax** (cities levy TPT, not income tax) → state-only withholding · **the 60-day nonresident rule + the CA/IN/OR/VA withholding exemption** · Phoenix semiconductor (TSMC/Intel) + construction boom, Tucson aerospace. areaServed-only, NO address. **TPT is NEVER called a "sales tax"** (honesty/accuracy). 5 children fanned out via 5 parallel subagents (GA children as exemplars), data-files-only. **Battery GREEN** (frozen copy). register_state.py arizona ran (6 URLs queued; arizona + arizona-advisory intents). **16/20 states.**
**🟢 TN CORE STATE COMPLETE (2026-06-26) — FULL battery green (flip-gate mode), pushed. WAVE C (Sunbelt GA·NC·AZ·TN) DONE.** Tennessee = **6 pages** (pillar + 5 essentials) on the AZ/GA/NC core-state exemplar. From `_build/strategy/TN-FACTS.md` (fact-checked vs Tennessee Dept of Revenue, cited): **NO individual income tax — the headline draw** (the **Hall income tax** on investment income was **fully repealed for tax years beginning Jan 1, 2021**, so payroll carries **NO state income-tax withholding** — a clean contrast to the AZ flat rate, OH municipal tax, and MI city tax) · **the Franchise & Excise (F&E) tax — the REAL business tax** (a **6.5% excise** on net earnings + a **0.25% franchise** tax on net worth, $0.25 per $100, **$100 minimum**; **net-worth-only since the 2024 property-measure/Schedule G repeal**; filed together on the FAE 170 — the TN-distinct business hook) · **the business (gross-receipts) tax** — a separate **state + city** tax on gross receipts at the **$100,000** threshold, rates by classification · **one of the highest combined sales taxes in the US** — a **7% state rate + a local option up to 2.75%** (multiples of 0.25, every locality has one; combined commonly **~9.25–9.75%**; specific local rates QUALITATIVE) · 95 counties / 3 Grand Divisions; Nashville healthcare-management (HCA) + music, Memphis logistics (FedEx), Knoxville/Oak Ridge, Chattanooga (VW), the auto/EV corridor (Nissan·VW·GM·Ford BlueOval City). areaServed-only, NO address. Authored data-files-only on existing v2 layouts (pillar + 5 children authored directly, AZ children as exemplars). **Battery GREEN** (frozen copy, FULL flip-gate mode): layout-v2 710, meta-unique 710/710, content-equity all PASS (additive), design-fidelity 704, 0 escaped-quote bugs, 0 dead links, no mojibake/entities. Founder-name-zero verified (Abid = global Person schema only; David Westgate = pillar operator card + #david-westgate @id, identical to AZ counts pillar 3/child 2). register_state.py tennessee ran (6 URLs queued; tennessee + tennessee-advisory intents). **17/20 states.**
**🟢 WA CORE STATE COMPLETE (2026-06-26) — FULL battery green (flip-gate mode), pushed. WAVE D STARTED.** Washington = **6 pages** (pillar + 5 essentials) on the TN/AZ core-state exemplar (5 children + pillar fanned out via 6 parallel subagents). From `_build/strategy/WA-FACTS.md` (fact-checked vs WA Dept of Revenue `dor.wa.gov` + ESD `esd.wa.gov` + WA Cares `wacaresfund.wa.gov`, cited): **NO income tax (individual OR corporate) — so NO state withholding** · **the Business & Occupation (B&O) tax = the REAL business tax, the WA-distinct hook** — a **gross-receipts** tax **by classification with NO deduction for any cost** (retailing 0.471%, wholesaling/manufacturing 0.484%, service tiered 1.5%/1.75%/higher-top eff Oct 1 2025), PLUS **city B&O** (Seattle/Tacoma/Bellevue/Everett levy their own — the extra layer; like AZ's TPT as the standout) · **high combined sales/use tax** (6.5% state + local, ~8.5–10.5%, destination-based; Oregon-border use-tax nuance) · **the 7% capital-gains tax** on owners (RCW 82.87; $278k 2025 deduction, +2.9% over $1M; real estate/retirement/business-use exempt — an INDIVIDUAL excise tax, advisory) · **WA payroll premiums** — PFML **1.13%** for 2026 (employer 28.57%/employee 71.43%, <50 employees exempt from employer share) + WA Cares **0.58%** employee-paid. 39 counties; Seattle (King; tech/maritime), Spokane, Tacoma, Vancouver (Clark; OR-border), Bellevue, Everett (Boeing), Kent, Olympia. areaServed-only, NO address; **schema sameAs `Washington_(state)`** (DC disambiguation — verified on all 6). The B&O is NEVER called an income or sales tax. **Battery GREEN** (frozen copy, FULL flip-gate): layout-v2 716, meta-unique 716/716, content-equity all PASS (additive), design-fidelity 710, 0 escaped-quote bugs, 0 dead links. Founder-name-zero verified. register_state.py washington ran (6 URLs queued; washington + washington-advisory intents). **18/20 states.**
**🟢 CO CORE STATE COMPLETE (2026-06-26) — FULL battery green (flip-gate mode), pushed. WAVE D continuing.** Colorado = **6 pages** (pillar + 5 essentials) on the WA/TN exemplar (6 parallel subagents). From `_build/strategy/CO-FACTS.md` (fact-checked vs Colorado Dept of Revenue `tax.colorado.gov` + CO General Assembly, cited): **flat 4.40% income tax** (individual AND corporate, 2025; temporarily 4.25% for 2024 via the TABOR-surplus mechanism — CO HAS an income tax, so payroll carries CO state withholding, a contrast to WA/TN) · **the home-rule self-collecting city sales tax = the CO-distinct hook** — the most fragmented sales-tax system in the US: state rate only **2.9%** but counties/cities/special districts stack (combined ~7–9%+), and **~70+ home-rule cities self-collect** with their OWN returns/registration (Denver, Colorado Springs, Aurora, Boulder, Fort Collins, Lakewood), many on the **SUTS** single-return portal, **destination-sourced** since Oct 1 2022 (the standout, analog to WA's city B&O) · the **retail delivery fee** ($0.28/delivery Jul 2025–Jun 2026, indexed; small/new businesses exempt) · the **SALT Parity PTE** election (advisory). 64 counties; Denver/Colorado Springs/Aurora/Fort Collins/Lakewood/Boulder/Pueblo/Greeley. areaServed-only, NO address; sameAs plain `Colorado`. **Battery GREEN** (frozen, FULL flip-gate): layout-v2 722, meta-unique 722/722, content-equity all PASS (additive), design-fidelity 716, 0 escaped-quote bugs, 0 dead links. Verified: 0 "no-withholding" leaks (CO state withholding correctly present), founder-name-zero. register_state.py colorado ran (6 URLs queued; colorado + colorado-advisory intents). **19/20 states.**
**🟢 MA CORE STATE COMPLETE (2026-06-26) — FULL battery green (flip-gate mode), pushed. 🎉 FINAL core state — WAVE D (WA·CO·MA) DONE, 20/20 STATES.** Massachusetts = **6 pages** (pillar + 5 essentials) on the CO income-tax-state exemplar (6 parallel subagents). From `_build/strategy/MA-FACTS.md` (fact-checked vs Massachusetts DOR `mass.gov/dor` + MA Dept of Family & Medical Leave, cited): **5% flat income tax + the 4% surtax = the MA-distinct hook** — the voter-enacted "millionaires"/Fair Share surtax on taxable income above an inflation-indexed threshold (**$1,083,150 for 2025 / $1,107,750 for 2026**; 9% on the excess), driving clean owner-comp/distribution/timing records + PTE planning · MA HAS income tax → MA state withholding (Form M-4), like CO · **the corporate excise** (an 8% net-income measure PLUS a property/net-worth measure of **$2.60 per $1,000**, **$456 minimum**) · **MA PFML** (2026 total **0.88%** of eligible wages, employer/employee split, employers <25 covered exempt from the employer share) · the **PTE election** (entity-level 5%, SALT-cap workaround, advisory) · a **flat 6.25% sales/use tax with NO local add-on** — its SIMPLICITY framed as the contrast to CO/WA. 14 counties; Boston (finance/biotech/healthcare), Cambridge (Kendall Sq biotech/MIT/Harvard), Worcester, Springfield, Lowell, Quincy, Newton, Framingham. areaServed-only, NO address; sameAs plain `Massachusetts`. **Battery GREEN** (frozen, FULL flip-gate): layout-v2 728, meta-unique 728/728, content-equity all PASS (additive), design-fidelity 722, 0 escaped-quote bugs, 0 dead links. Verified: 0 "no-withholding" leaks (MA state withholding correctly present), surtax present on all 6, founder-name-zero. register_state.py massachusetts ran (6 URLs queued; massachusetts + massachusetts-advisory intents). **20/20 states.**
**STATES: 🎉 20 built / 20 planned — COMPLETE** (5 existing NY·CA·TX·FL·IL + DE + IN + PA + NJ + MD + VA + OH + MI + GA + NC + AZ + TN + WA + CO + **MA**). **NO states remain.** **NEXT: the national-hub capstone** (rebuild `/find-an-accountant/` as the hub linking all 20 state pillars; drop the legacy 301 — a ROUTING change, verify URL set unchanged + battery before commit) → T4 FAQ remainder → sitewide "verifiable on Intuit" honesty sweep (~50) → T6 flip gates.
...
**✅ ENTITY-ESCAPE FIX DONE [a329c24]:** founder green-lit; converted HTML entities -> literal Unicode in the auto-escaped
...
**✅ PRE-PUBLISH AUDIT (A-E) DONE [4871c83 A+B · 9320662 C+E · 550863a log]:** A standing checklist recorded · B mojibake+entities fixed + `meta-hygiene` battery gate · C head/meta structurally clean (canonical/OG/robots-noindex-no-leak/lang all good; `_site-prod` emits index,follow) · D schema fully clean+complete (Org/WebSite/Service/FAQPage/Breadcrumb/BlogPosting all present+valid, 0 fabricated) · E fixed 3 homepage 404s + sitemap 528→580 (glossary+blog pagination addAllPagesToCollections). **Report-only Task E (founder call later): 33 orphan pages · 16 homepage 301-hop state links · robots/sitemap flip gate · prod-env flip gate.**
**🔑 FOUNDER DECISIONS (2026-06-25):** (1) title separator = KEEP `·` (no change). (2) Sequence = **TITLE/DESC REWRITE FIRST**, then David block + FAQ (TASK 2/3/5).
**🗂 MASTER TASK SET (2026-06-25) — canonical tracker now `MASTER-TASKLIST.md`.** Order: T1 form✅ → T2 title/desc → T3 David → T4 FAQ → T5 locations(PLAN-FIRST, founder review) → T6 cleanup/flip. Portable strategy docs in `_build/strategy/`: FAQ✅ PRE-PUBLISH✅ FORM✅ · TODO META/LOCATION-SEO/EEAT. **T1 FORM TRIM DONE:** both forms stripped of months/qb_status/lead_source/ai_recommended/ai_tool visible fields; lead_source/ai_tool/ai_recommended now hidden auto-captured (tb-forms.js deriveAttribution from URL+referrer); minimal=name/email/convincing/message; full kept company/phone/services/revenue/industry/timeline (interp flag: confirm if full should reduce to 4). Also optimized meta-hygiene battery check → title/desc-only (T6 item). **FOUNDER DECISIONS PENDING: full-form trim scope · T5 LOCATION PLAN (hard-stop before building).**
**T2 ~COMPLETE (~517 meta rewrites):** META-STRATEGY.md · `meta-unique` gate · `meta-hygiene` opt · `--fast` battery mode added (58678f3; provisional cadence gate, FULL run before flip). Waves: w1 6 · w2 industries 26 · w3 (help+support 57/online 42/glossary 46=145) · w4 (find-an-accountant 174/qb-rest 65/accounting+standalones 38/resources-vs-switch-pricing-tools-blog 63=340). Sitewide: 0 dup titles/descs, 0 hygiene fails. Stragglers: blog/category.njk (data-driven, paginated) — verify/address if material.
**T3 DONE:** David operator block authored on /about/ (#about-operator) + /partners/ (#partners-operator) — contextual, real bio, photo; reviewedBy #david-westgate added to both schema nodes; /trust/ untouched (team-voice). EEAT-STRATEGY.md recorded. blog/category meta straggler fixed (8 descs now ≤160, boilerplate+phone removed). Verified on built pages; --fast content-equity clean.
**T4 FAQ IN PROGRESS:** money done (31 pages: cleanup proof d6927fe + 30-page wave 3fb0f8b) — each 7 FAQs, call-intent FAQ + phone + silo links, answer-first. 🚩 FLAGGED for founder: NY (12) + migration-child (8) pages had OLDER frozen-FAQ notes; current ruling supersedes, 1-2 redundant Qs/page replaced to fit cap + add call-intent (0 equity failures — growth pages; reversible). 2 judgment cuts (NY NYC-rate kept in aiSummary; fractional-cfo) for optional revisit. NOTE: --fast SKIPS faq checks → FAQ waves need full run / targeted faq verify before flip.
**T4 money DONE (Option C, 219ca41):** founder ruled OPTION C (FAQ-STRATEGY.md rule 5, f82cb27) — phone woven into an EXISTING FAQ answer (equity-safe); dedicated call-intent Q only where cap room + no baseline removal; NEVER reword/remove a baseline FAQ question. (First non-Option-C attempt removed baseline Qs → 26 content-equity fails → reverted fb09341/040a5dd; my error from committing on a deleted/misread battery output.) Redo: 31 money pages, questions byte-identical, content-equity run-to-completion + read = 0 fails. PROCESS FIX: content-equity to completion + READ before commit; FULL battery before flip (--fast skips faq-schema/overlap/flat).
**▶ NOW: T4 remaining clusters via Option C (prose · pillars · location children · comparison · /faq/ hubs EXEMPT · t-hub FAQ ADDITIONS · silo links) → T5 LOCATION PLAN (HARD-STOP for founder review) → T6 flip gates (FULL battery run).** (standing rule in PRE-PUBLISH-AUDIT.md: title LEADS with keyword · brand `· TechBrot` only if ≤60 else DROP · unique · desc lead value+keyword ≤160 · phone on high-intent[done]). Scope: 263 titles >60 + 511 descs >160 (the 52 high-intent descs already tight from TASK 4). Per page-type, highest-value first, authored not bulk-stamped, battery+commit per wave. THEN:
...
quickbooks 30 (hubs online/desktop/enterprise/help + sub-pages + compare 7 + payroll 5 + support 3) ·
accounting non-industry ~15 (advisory 6 / services 3 / misc 6 / faqs) · switch 4 · standalones ~5
(app-stack/which-accounting-software/reviews/platforms/404). Mostly bespoke → individual migration (check for
shared partials first).
