...
**▶ NEXT (autonomous): remaining T4 FAQ clusters (Option C — answer-first, fold call-intent FAQs w/ phone, never remove a baseline FAQ) → sitewide "verifiable on Intuit" honesty sweep (~50 occurrences OUTSIDE the location silo; founder keeps DE-GBP) → T6 flip gates (FULL battery, noindex-leak check, baseline-vs-prod recrawl, prod-env proof).** State-build exemplars for any future work: the built states under `src/find-an-accountant/<state>/` + their `_build/strategy/<XX>-FACTS.md`.
